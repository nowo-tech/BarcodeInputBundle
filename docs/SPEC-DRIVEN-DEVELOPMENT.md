...
2. **Product behavior** — what **BarcodeInputBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** form author, **I want** a barcode input with optional camera scan **so that** users capture product or warehouse codes easily. |
| US-02 | **As an** integrator, **I want** a single Symfony form field value **so that** validation and submission stay standard. |
| US-03 | **As an** integrator, **I want** configurable formats, max length, and scanner toggle **so that** the widget matches my inventory flow. |
| US-04 | **As a** maintainer, **I want** Vitest/PHPUnit in CI **so that** normalization and scanner behavior do not regress. |
...
**Goal:** Symfony Barcode FormType with optional camera scanner and hardware wedge support mapped to a single normalized field value.
